...
angular.module('ceDirectives', [ 'ceConf', 'ceServices', 'ceCtrls' ])
	...
	.directive('ceMkdirBtn', function()
	{
		return {
			restrict: 'C',
			template: '<button class="new_folder" ng-click="mkdir()">New folder</button>',
			replace: 'true',
			...
			template: "<div> \
						<ul class=\"tree\"> \
							<li ng-repeat=\"srvIt in srvList\" ng-init=\"srv=srvIt.name; path='';\"> \
								<span class=\"ce-item ce-folder srvIt\" ng-click=\"enterDir()\"><img title=\"{{ srvIt.description }}\" ng-src=\"../{{ srvIt.image_small }}\" /> <span class=\"tool_name\">{{ srvIt.display_name }}</span></span> \
							</li> \
						</ul> \
					</div>",
		...
		return {
			...
			template: "<div> \
						...
						<div ng-if=\"isConnected()\" class=\"ce-item ce-folder dropzone\" data-path=\"{{ filePath = path }}\"> \
							...
							<h2 class=\"drop_text\">Drop your file(s) here</h2> \
						</div> \
					</div>",
//								<a ng-hide=\"file.is_dir\" ng-href=\"{{download()}}\" download=\"{{file.name}}\" target=\"blank\">download</a> \
	...
	.directive('ceBrowser',  function()
	{
		return {
			...
			replace: true,
			template: "<div> \
						<div class=\"ceTitle\">Browse your cloud <span class=\"bold_text\">drives</span> <button class=\"close-btn\" type=\"button\" ng-click=\"abort()\"></button> <button class=\"color-btn\" type=\"button\" ng-click=\"logout()\" ng-if=\"isConnected()\"> <i class=\"fa fa-power-off color-btn\"></i> ({{ getDisplayName() }}) </button></div> \
						<div ng-if=\"isPopupBlocked()\" class=\"error-popup\">Popup Blocker is enabled! Please add this site to your exception list.</div> \
						<div ng-if=\"authPopup.show\" class=\"authPopup\"><div>Would you like to authorize <span class=\"bold_text\">Cloud Explorer</span> to use your {{ authPopup.srvName }} account ? <br/><br/><a class=\"style_btn green_btn\" ng-click=\"authorize()\" >YES, I'M OKAY WITH THAT !</a><a class=\"style_btn red_btn\" href=\"#\" >NO, PLEASE !</a></div></div> \
						<div class=\"row-fluid\"> \
							<div class=\"span5\"> \
