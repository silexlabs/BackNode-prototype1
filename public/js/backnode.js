...
BackNode.prototype.editor = {
  ...
  editPicture: function(picture) {/*need to be modified, doesn't active now, that's so dirty */
      ...
      var contentPopinpicture = '<form name="bn-picForm" style="border-radius:3px;-moz-border-radius:3px;-webkit-border-radius:3px;box-shadow:2px 2px 1px #000;width:500px;height:500px;text-align:center;background:#eee;position:absolute;left:50%;top:50%;margin:-250px 0 0 -250px"><div style="padding:5px;margin-bottom:20px;color:#000;display:block;text-align:center">Import your file<span style="cursor:pointer;position:absolute;top:0;right:0"><img src="'+ document.URL +'/img/close-pic.png" alt="X" /></span></div><div><label for="bn-picSrc">Src attribute</label><input id="bn-picSrc" type="text" name="picSrc" value="'+ picture.attr('src') +'" /></div><div><label for="bn-picAlt">Alt attribute</label><input id="bn-picAlt" type="text" name="picAlt" value="'+ picture.attr('alt') +'" /></div><button id="bn-picUpload">Upload a file</button><button id="bn-valid">Valid</button</div>';
